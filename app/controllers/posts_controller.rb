class PostsController < ApplicationController
  ...
  # GET /posts.json
  def index
    # show all the posts in descending order by the timestamp field using .order('timestamp DESC')
    @posts = Post.order('timestamp DESC').paginate(:page => params[:page], :per_page => 30)
    # for use with the filter on the view index page
    ...
    # testing json/ api format in my application
